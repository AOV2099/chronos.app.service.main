...
function formatHorariosToProfesores(horarios, semestres = SEM_LIST) {
  const profMap = new Map();
  for (const h of horarios || []) {
    const name = (h.profesor ?? "SIN NOMBRE").toString().trim();
    const rfc = (h.rfc ?? "N/D").toString().trim();
    const worker = (h.numTrabajador ?? "N/D").toString();
  ...
  for (const [, p] of profMap) {
    ...
    profesores.push({ name: p.name, rfc: p.rfc, worker: p.worker, subjects });
  }
  return profesores;
}
...
function header(doc, logos = {}) {
  ...
  const boxW = 80,
    boxH = 80;
  const boxY = 40; // posicion vertical de los logos
  const leftX = MARGIN;
...
/*function drawThreeRowHeader(doc, topY) {
  ...
  drawTable(doc, {
    ...
    cells: [
      [
        { text: "ACADÉMICO", align: "center", bold: true },
        { text: "NOMBRAMIENTO", align: "center", bold: true },
  ...
  for (let i = 1; i < 5; i++) {
    const x = subXs[i];
    doc
      .moveTo(x, topY + BAND_H)
      .lineTo(x, topY + BAND_H * 3)
      .stroke();
  }
  doc.restore();
  ...
  sems.forEach((s, i) => {
    const x = subXs[i + 1];
    const w = subWs[i + 1];
    doc.text(s, x, topY + BAND_H +2 + (BAND_H / 2 - 6), {
      width: w,
      align: "center",
    });
  });
  ...
  sems.forEach((_, i) => {
    const x = subXs[i + 1];
    const w = subWs[i + 1];
    doc.text("HT / HP", x, topY + 2 * BAND_H + (BAND_H / 2 - 5), {
      width: w,
      align: "center",
    });
  });
...
function drawThreeRowHeader(doc, topY) {
  ...
  // Separadores verticales A | B | Right
  ...
  doc
    .moveTo(MARGIN, y1)
    .lineTo(MARGIN + INNER_W, y1)
    .stroke();

  // Horizontal tras la 2ª fila SOLO en los semestres (omitiendo PARCIAL)
  doc
    .moveTo(xRight + wParcial, y2)
    .lineTo(MARGIN + INNER_W, y2)
    .stroke();
  ...
  const subX = [
    xRight + wParcial, // <-- frontera PARCIAL | Semestre 1 (restaurada)
    xRight + wParcial + wSem,
    xRight + wParcial + 2 * wSem,
    xRight + wParcial + 3 * wSem,
  ];
  // Trazamos todas menos el borde izquierdo del bloque (que ya está con xBEnd)
  ...
  sems.forEach((_, i) => {
    const x = xRight + wParcial + i * wSem;
    doc.text("HT / HP", x, y2 + (BAND_H / 2 - 5), {
      width: wSem,
      align: "center",
    });
  });
...
function drawRow(doc, topY, { nameInfo, nomText, kind, values }) {
  ...
  if (nameInfo) {
    ...
    doc
      .font("Helvetica")
      .fontSize(TABLE_FONT.nameMeta)
      .text(`RFC: ${rfc}`, MARGIN + 10, metaY, {
        width: nameBoxW,
        align: "left",
      })
      .text(`NÚMERO DE TRABAJADOR: ${worker}`, MARGIN + 10, metaY + 10, {
        width: nameBoxW,
        align: "left",
      });
  }
  ...
  if (nomText) {
    doc
      .font("Helvetica")
      .fontSize(TABLE_FONT.body)
      .fillColor(C.text)
      .text(nomText, MARGIN + COL_A_W + 10, topY + 10, {
        width: COL_B_W - 20,
        align: "left",
      });
  }
...
router.get("/horario-pdf", async (req, res) => {
  ...
  y = drawThreeRowHeader(doc, y - 48);//altura del header
  ...
  // Formatear horarios -> estructura de profesores
  let profesores = formatHorariosToProfesores(horarios, SEM_LIST);
    ...
    profesores = [
      {
        ...
        subjects: [
          {
            nom: 'PROFESOR DE ASIGNATURA "A" INTERINO',
            parcial: SEM_LIST.map(() => [0, 0]),
          },
        ],
      },
    ];
